...
**🔒 세션 관리 시스템** ✅
- **30분 자동 세션 유지**: 로그인/회원가입 시 자동으로 30분 세션 생성
- **실시간 세션 타이머**: 화면 우측 상단에 남은 시간 표시 (예: `포워더 | 이영희 | 29:59`)
- **시각적 카운트다운**:
  - 파란색: 5분 이상 남음
  - 주황색: 1-5분 남음
  - 빨간색: 1분 미만 남음
- **2분 전 경고**: 세션 만료 2분 전 경고 알림 표시
- **자동 로그아웃**: 30분 경과 시 자동 로그아웃 및 로그인 페이지로 리다이렉트
- **페이지 간 세션 유지**: 모든 페이지에서 세션 상태 자동 동기화
- **새로고침 후에도 유지**: localStorage 기반 세션 관리로 새로고침 시에도 세션 유지
- **세션 타이머 표시 위치**: 네비게이션 바 아래 (z-index: 40)
- **적용 대상**:
  - ✅ 비밀번호 로그인 (이메일/패스워드)
  - ✅ 블록체인 해시키 로그인
  - ✅ 회원가입 (즉시 로그인 처리)
...
- **🆕 동적 도착지 필터링** - 출발지 입력 시 연결된 항구만 표시
- 출발지/도착지 검색 (한글/영문 모두 지원)
- 자동완성 datalist (출발지 입력 시 자동 추천)
- 실시간 도착지 개수 표시 (예: `도착지 (3개 항구)`)
- 날짜 필터
- 컨테이너 타입 필터
- 가격 정렬 (낮은/높은 순)
- 선박 카드 목록 표시
...
- `GET /api/vessels/destinations/:departure` - 출발지별 연결 도착지 목록 🆕
- `GET /api/vessels/departure-ports` - 모든 출발지 목록 🆕
...
### 🆕 새 계정 만들기 (권장)
**회원가입으로 자신만의 계정을 생성하세요!**
- URL: `/signup`
- 필수 정보: 이메일, 비밀번호, 이름, 역할 (화주/포워더/선사)
- 선택 정보: 회사명, 전화번호
- ✅ **회원가입 즉시 로그인 처리** (30분 세션 자동 생성)
- ✅ **세션 타이머 자동 시작** (화면 우측 상단 표시)
- ✅ **모든 기능 즉시 사용 가능** (Verified 권한)
...
### 매직 링크 로그인 (간편 로그인)
- 이메일만으로 간편하게 로그인
- 계정이 없으면 자동으로 Basic 권한 계정 생성
- 6자리 인증 코드 입력 (5분 유효)
- **권한**: Basic (월 10회 검색 제한)
- **업그레이드**: Basic → Verified 전환 가능
...
**🎉 최근 완료 기능:**
- **MTIS API 통합** (한국해양교통안전공단) 🆕
  - 실시간 선박 위치 추적 준비
  - 부산항/인천항 입출항 정보 API
  - 여객선 운항 정보 (PATIS)
  - 해양교통량 모니터링
- 동적 도착지 필터링 (출발지 입력 시 연결 항구만 표시)
- 출발지 자동완성 및 도착지 개수 표시
- 한글/영문 양방향 항구 이름 검색 지원
...
### 동적 날짜 생성 시스템
선박 데이터는 **항상 오늘 날짜 기준**으로 생성됩니다:
- 출발일: 오늘부터 +30일까지
- 10개 선박이 2주에 걸쳐 분산 배치
- 날짜 수동 관리 불필요
...
### 검색 가능 기간
- **과거 데이터**: 표시 안 됨 (자동 필터링)
- **현재~미래**: 오늘부터 한 달간 검색 가능
- **날짜 필터**: "출발일 이후" 조건으로 검색

### 예시 일정 (2025-12-01 기준)
```
오늘 (12-01):    HAPAG EXPRESS (부산→LA)
내일 (12-02):    MAERSK ESSEX (부산→LA)
모레 (12-03):    MSC OSCAR (부산→LA)
...
+10일 (12-11):   CMA CGM ANTOINE (평택→뉴욕)
```

### 주의사항
- 하루가 지나면 `npm run db:seed` 실행 권장
- 로컬 개발 환경에서만 작동 (`--local` 플래그)
- 프로덕션 배포 시 별도 스케줄러 필요
